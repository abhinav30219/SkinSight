"""
Gradio web interface for skin lesion diagnosis using SimpleViT.
...
class SkinLesionDiagnosisApp:
    ...
    def load_model(self):
        ...
        try:
            self.model = SimpleViTDiagnostic.from_pretrained(self.model_path, self.device)
            self.model.eval()
    ...
    def create_probability_chart(self, probabilities: dict) -> plt.Figure:
        ...
        # Add OOD threshold line
        ax.axvline(x=self.model.ood_threshold, color='blue', linestyle='--', alpha=0.5, 
                  label=f'OOD threshold ({self.model.ood_threshold:.0%})')
        ax.legend()
    ...
    def predict(self, image):
        ...
        try:
            ...
            # Get prediction
            result = self.model.diagnose(image, return_probabilities=True)
    ...
    def format_prediction_result(self, result: dict) -> str:
        ...
        requires_review = result.get('requires_professional_review', False)
        
        # Check if it's an OOD detection
        is_ood = "uncertain" in diagnosis.lower() or "unknown" in diagnosis.lower()
...
**Professional Review Required:** {'Yes ⚠️' if requires_review else 'Recommended'}
"""
        
        # Add description if not OOD
        if not is_ood and diagnosis in self.class_info:
            info = self.class_info[diagnosis]
            output += f"\n**Description:** {info['description']}\n"
        
        # Add probability table if available
        if 'probabilities' in result:
            output += "\n## All Probabilities:\n"
...
The model has low confidence in this image, which may indicate:
- The image is not a dermatoscopic image
- The skin condition is not one of the 7 trained categories
- The image quality is poor or unusual
...
"""
    ...
    def create_confidence_indicator(self, confidence: float) -> str:
        ...
        if confidence >= 0.8:
            ...
            color = "#FF9800"  # Orange
            level = "Moderate"
        elif confidence >= self.model.ood_threshold:
            color = "#FF5722"  # Deep Orange
            level = "Low"
        else:
            color = "#F44336"  # Red
            level = "Very Low (Possible OOD)"
    ...
    def create_interface(self):
        ...
        **Model Performance:** ~49% test accuracy with out-of-distribution detection capability.
        """
        
        # Example images info
        examples_info = """
        ### About the Model
        This diagnostic tool uses a Vision Transformer (ViT) model fine-tuned on the HAM10000 dataset.
        ...
        The model includes out-of-distribution detection to identify images it's uncertain about.
        """
    ...
    def get_example_images(self):
        ...
        if examples_dir.exists():
            # Get a diverse set of example images
            import random
            example_files = list(examples_dir.glob("*.jpg"))
            random.shuffle(example_files)
            return [str(f) for f in example_files[:8]]
        return []
